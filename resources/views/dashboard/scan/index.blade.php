        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="border-t pt-4">
                    ...
                    @csrf                    <div>
                        ...
                        <div class="relative">
                            <input type="text" 
                                   id="qrCodeInput" 
                                   name="qr_code"
                                   class="w-full h-16 text-center text-lg font-bold border-2 border-blue-300 rounded-lg focus:border-blue-500 focus:outline-none bg-blue-50 uppercase tracking-wide" 
                                   placeholder="ABCD1234 atau URL"
                ...
                <div class="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                    <p class="text-xs font-medium text-gray-600 mb-2">💡 Contoh yang bisa dimasukkan:</p>
                    <div class="space-y-2">
                        <div class="flex justify-center">
                            <span class="bg-blue-100 border border-blue-300 px-3 py-2 rounded font-mono text-blue-800 text-sm font-bold">MTKL8264</span>
                        </div>
                        <div class="text-center text-xs text-gray-500">atau</div>
                        <div class="flex justify-center">
                            <span class="bg-green-100 border border-green-300 px-3 py-2 rounded font-mono text-green-800 text-xs">http://127.0.0.1:8000/absensi/MTKL8264</span>
                        </div>
                    </div>
                </div>
            </div>
...
function initializeApp() {
    ...
    // Stop Scanner
    stopBtn.addEventListener('click', stopScanner);    // Manual Form Submit
    manualForm.addEventListener('submit', function(e) {
        e.preventDefault();
        const inputText = inputElement.value.trim();
        const qrCode = extractQrCodeFromText(inputText);
    ...
    });    // Input validation - allow URL input now
    inputElement.addEventListener('input', function(e) {
        // Just trim and update validation, no more character restrictions
        validateInput();
    });
    ...
    html5QrcodeScanner = new Html5QrcodeScanner(
      ...
      html5QrcodeScanner.render(
        function(decodedText, decodedResult) {
            ...
            const extractedCode = extractQrCodeFromText(decodedText);
            console.log('Extracted QR Code:', extractedCode);
...
function validateInput() {
    ...
    const extractedCode = extractQrCodeFromText(inputText);
    const isValid = extractedCode.length === 8 && /^[A-Z]{4}[0-9]{4}$/.test(extractedCode);
...
function extractQrCodeFromText(text) {
    console.log('Extracting QR code from text:', text);
    
    // If text is already 8 characters and matches pattern, return as is
    const cleanText = text.trim().toUpperCase();
    if (cleanText.length === 8 && /^[A-Z]{4}[0-9]{4}$/.test(cleanText)) {
        console.log('Text is already valid QR code:', cleanText);
        return cleanText;
    }
    
    // Try to extract from URL patterns
    // Pattern 1: http://domain/absensi/XXXX#### or http://domain/path/XXXX####
    const urlPattern = /(?:https?:\/\/[^\/]+(?:\/[^\/]*)*\/)([A-Z]{4}[0-9]{4})(?:\/.*)?$/i;
    const urlMatch = text.match(urlPattern);
    if (urlMatch) {
        const extracted = urlMatch[1].toUpperCase();
        console.log('Extracted from URL pattern:', extracted);
        return extracted;
    }
    
    // Pattern 2: Look for 8-character pattern anywhere in the text
    const anywherePattern = /([A-Z]{4}[0-9]{4})/i;
    const anywhereMatch = text.match(anywherePattern);
    if (anywhereMatch) {
        const extracted = anywhereMatch[1].toUpperCase();
        console.log('Extracted from anywhere in text:', extracted);
        return extracted;
    }
    
    // Pattern 3: Extract from end of URL (last 8 characters if they match pattern)
    if (text.length > 8) {
        const lastEight = text.slice(-8).toUpperCase();
        if (/^[A-Z]{4}[0-9]{4}$/.test(lastEight)) {
            console.log('Extracted from end of text:', lastEight);
            return lastEight;
        }
    }
    
    // If nothing found, return original text (will fail validation later)
    console.log('Could not extract valid QR code, returning original:', text);
    return text;
}
